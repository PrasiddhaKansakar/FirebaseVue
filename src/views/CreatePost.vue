<template>
  <div class="create-post">
    <ProductPreview v-show="this.$store.state.productPhotoPreview" />
    ...
    <div class="container">
      ...
      <div class="product-info">
        <input
          type="text"
          placeholder="Enter Product Name & Price"
          v-model="productTitle"
        />
        <div class="upload-file">
          <label for="product-photo">Upload Cover Photo</label>
          <input
            type="file"
            ref="productPhoto"
            id="product-photo"
      ...
      <div class="product-actions">
        <button @click="uploadProduct">Publish Product</button>
...
export default {
  ...
  methods: {
    ...
    uploadProduct() {
      if (this.productTitle.length !== 0 && this.productHTML.length !== 0) {
        if (this.file) {
          this.loading = true
          const storageRef = firebase.storage().ref()
          const docRef = storageRef.child(
            `documents/ProductCoverPhotos/${this.$store.state.productPhotoName}`
          )
          docRef.put(this.file).on(
            ...
            async () => {
              ...
              const dataBase = await db.collection('productPosts').doc()
              await dataBase.set({
      ...
      this.error = true
      this.errorMsg =
        'Please ensure Product Title & Product Post has been filled!'
      setTimeout(() => {
        this.error = false
      }, 5000)
      return
